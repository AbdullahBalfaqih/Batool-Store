...
export default function Stepper({
  ...
  const handleBack = () => {
    if (currentStep > 1) {
      setDirection(-1);
      updateStep(currentStep - 1).catch(() => {}); // Catch potential rejections
    }
  };
  ...
  return (
    ...
    >
      <div
        className={`mx-auto w-full max-w-md rounded-2xl bg-surface-dark shadow-xl ${stepCircleContainerClassName}`}
          ...
          <div className={`px-8 pb-8 ${footerClassName}`}>
            <div className={`mt-10 flex ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`}>
              ...
              <button
                ...
                className="duration-350 flex items-center justify-center rounded-full bg-primary py-1.5 px-3.5 font-medium tracking-tight text-white transition hover:bg-primary/90 active:bg-primary/80 disabled:opacity-50"
                {...nextButtonProps}
              >
                {isLastStep ? 'إرسال الطلب' : nextButtonText}
              </button>
            </div>
          </div>
        )}
    ...
    >
      <motion.div
        variants={{
          ...
          complete: { scale: 1, backgroundColor: 'hsl(var(--primary))', color: '#fff' }
        }}
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    incomplete: { width: 0, backgroundColor: '#333' },
    complete: { width: '100%', backgroundColor: 'hsl(var(--primary))' }
  };

  return (
    <div className="relative mx-2 h-1 flex-1 overflow-hidden rounded bg-neutral-700">
      <motion.div
